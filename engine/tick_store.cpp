                               ...
                               std::int64_t end_time)
{
    ...
    if (it != cracking_index.end()) {
        part_start = it->second.start;
        part_length = it->second.length;
        std::cout << "[crack_and_query] Cache hit. Reusing partition [" 
                  << part_start << ", " << part_start + part_length << ").\n";
    } else {
        std::size_t    left  = next_free;
        ...
        part_start = next_free;
        part_length = partition_end - next_free;
        last_partition_size = part_length;

        cracking_index[target_symbol] = PartitionInfo{part_start, part_length, part_length <= 1};
        next_free = partition_end;

        std::cout << "[crack_and_query] Partition complete.  "
                  << part_length << " ticks matched symbol " << target_symbol << ".\n";
    }

    double sum = 0.0;
    std::size_t match_count = 0;

    for (std::size_t i = part_start; i < part_start + part_length; ++i) {
    ...
    std::cout << "[crack_and_query] Scanned " << part_length
...
double Engine::simd_cracked_query(std::size_t  part_start,
                                  ...
                                  std::int64_t end_time) const
{
    ...
    const std::size_t part_end = part_start + part_size;
    ...
    std::size_t i        = part_start;
    std::size_t simd_end = (part_size >= 8) ? (part_end - 7) : part_start;
    ...
    std::cout << "[simd_cracked_query] Scanned " << part_size
              << " ticks via AVX2 (partition [" << part_start
              << ", " << part_end << ")) — " << match_count
              << " in time window.  Average price = " << average << "\n";
                                ...
                                std::int64_t end_time)
{
    ...
    if (it != cracking_index.end()) {
        ...
        PartitionInfo& pi = it->second;

        std::cout << "[Index] Cache hit for symbol " << target_symbol
                  << ". Reusing partition [" << pi.start
                  << ", " << (pi.start + pi.length)
                  << ") — " << pi.length << " rows. Skipping cracking.\n";

        if (!pi.is_sorted) {
            if (pi.length > 1) {
                sort_partition_by_time(pi.start, pi.start + pi.length - 1);
            }
            pi.is_sorted = true;
            std::cout << "[Sort] Chronologically sorted partition for symbol "
                      << target_symbol << ".\n";
        }

        auto [range_start, range_end] = binary_search_time(
            pi.start, pi.length, start_time, end_time);

        std::size_t scan_size = (range_end > range_start)
                              ? (range_end - range_start) : 0;

        std::cout << "[Search] Binary search narrowed scope to "
                  << scan_size << " rows within partition.\n";
        ...
        return simd_cracked_query(range_start, scan_size,
                                  start_time, end_time);
        ...
        std::size_t part_start  = next_free;
        std::size_t part_length = partition_end - next_free;

        last_partition_size = part_length;

        cracking_index[target_symbol] = PartitionInfo{
            part_start,
            part_length,
            part_length <= 1
        };
        ...
        std::cout << "[Index] Cache miss. Cracked tail [" << part_start
                  << ", " << partition_end << ") for symbol "
                  << target_symbol << " — " << part_length
                  << " rows.  next_free is now " << next_free << ".\n";

        if (part_length == 0) {
            std::cout << "[smart_simd_query] Symbol " << target_symbol
                      << " has no ticks in the dataset.\n";
            return 0.0;
        }

        if (part_length > 1) {
            sort_partition_by_time(part_start, partition_end - 1);
        }
        cracking_index[target_symbol].is_sorted = true;
        std::cout << "[Sort] Chronologically sorted partition for symbol "
                  << target_symbol << ".\n";

        auto [range_start, range_end] = binary_search_time(
            part_start, part_length, start_time, end_time);
